...
function GlobalSearch() {
  ...
  const results = useMemo(() => {
    ...
    if (!term) return [];
    const out: { label: string; kind: string; to: string }[] = [];
    (leadsQuery.data ?? [])
...
function NotificationBell() {
  ...
  const readMutation = useMutation({
    ...
    onError: (e) =>
      toast.error(e instanceof Error ? e.message : "Unable to mark notification as read"),
  });
  ...
  const handle = (n: AppNotification) => {
    ...
    if (n.type === "FOLLOW_UP") {
      if (followUpsQuery.isPending || (followUpsQuery.data ?? []).some((f) => f.id === n.targetId)) {
        setOpen(false);
        navigate({ to: "/calendar", search: { followUpId: n.targetId } });
      } else {
    ...
    if (route) navigate({ to: route, params: { id: n.targetId } });
  };
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-[min(92vw,22rem)] p-2">
        ...
        {notificationsQuery.isPending ? (
          ...
          <p className="p-3 text-sm text-destructive">Unable to load notifications.</p>
        ) : notifications.length === 0 ? (
          <p className="p-3 text-sm text-muted-foreground">No notifications.</p>
        ) : (
          <div className="mt-1 max-h-[min(60vh,24rem)] overflow-y-auto space-y-1">
            {notifications.map((n) => (
              <button
                key={n.id}
                onClick={() => handle(n)}
                disabled={readMutation.isPending && readMutation.variables === n.id}
                className={cn(
                  "block w-full rounded-lg px-3 py-2 text-left transition-colors",
                  !n.read ? "bg-mint/30" : "hover:bg-muted",
                  readMutation.isPending && readMutation.variables === n.id && "opacity-60",
                )}
              >
...
function setSidebarState(collapsed: boolean) {
  if (typeof window !== "undefined") window.localStorage.setItem(SIDEBAR_KEY, String(collapsed));
  ...
  return (
    <TooltipProvider delayDuration={100}>
      <div className={cn("flex min-h-screen w-full flex-col lg:flex-row", plain && "bg-white")}>
        <aside
          className="sidebar-glass fixed inset-y-0 left-0 z-40 hidden h-screen shrink-0 flex-col gap-2 border-r border-sidebar-border bg-sidebar p-3 backdrop-blur-xl transition-[width] duration-300 ease-out lg:flex"
