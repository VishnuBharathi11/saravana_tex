...
export function FollowUpDetailDialog({ detail, onClose }: { detail: FollowUp | null; onClose: () => void }) {
  ...
  return <Dialog open={!!detail} onOpenChange={(o) => !o && onClose()}><DialogContent className="max-w-lg bg-white"><DialogHeader><DialogTitle>{current.title}</DialogTitle></DialogHeader>{loading ? <p className="py-6 text-sm text-muted-foreground">Loading follow-up...</p> : detailQuery.isError ? <p className="py-6 text-sm text-destructive">{detailQuery.error instanceof Error ? detailQuery.error.message : "Unable to load follow-up."}</p> : <div className="space-y-2 text-sm"><p className="text-muted-foreground">{current.description}</p><div className="grid grid-cols-2 gap-2">{[["Date", formatDate(current.date)], ["Time", formatTime(current.time)], ["Related", `${current.relatedType}: ${current.relatedName}`], ["Owner", employee?.name ?? "Unassigned"], ["Priority", current.priority], ["Reminder", current.reminder ? "On" : "Off"]].map(([k, v]) => <div key={k} className="rounded-lg border px-3 py-2"><p className="text-[11px] uppercase text-muted-foreground">{k}</p><p className="text-sm font-medium">{v}</p></div>)}</div><div className="rounded-lg border px-3 py-2"><p className="text-[11px] uppercase text-muted-foreground">Previous follow-ups</p>{previous.length === 0 ? <p className="mt-1 text-xs text-muted-foreground">No previous follow-ups.</p> : previous.map((f) => <p key={f.id} className="mt-1 text-xs">{formatDate(f.date)} · {f.title} · {f.status}</p>)}</div>{canEditRecord(user, current) && <div className="flex gap-2 pt-1"><Button className="rounded-xl" disabled={current.status === "Completed" || completeMutation.isPending} onClick={() => completeMutation.mutate(current.id)}>{completeMutation.isPending ? "Completing..." : "Mark completed"}</Button><Button variant="ghost" className="rounded-xl" onClick={() => { onClose(); navigate({ to: current.relatedType === "Lead" ? "/leads/$id" : current.relatedType === "Order" ? "/orders/$id" : "/customers/$id", params: { id: current.relatedId } }); }}>View {current.relatedType}</Button><Button variant="ghost" className="rounded-xl text-destructive" disabled={deleteMutation.isPending} onClick={() => deleteMutation.mutate(current.id)}>{deleteMutation.isPending ? "Deleting..." : "Delete"}</Button></div>}</div>}</DialogContent></Dialog>;
}
