...
import { getCustomer, updateCustomer, deleteCustomer, type UpdateCustomerInput } from "@/api/customers";
...
function CustomerDetail() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => toast.error(error instanceof Error ? error.message : "Unable to update customer"),
  });
  const deleteMutation = useMutation({
    ...
    onError: (error) => { setConfirmDelete(false); toast.error(error instanceof Error ? error.message : "Unable to delete customer"); },
  });
  if (!user) return null;
  ...
  return <AppShell><div className="space-y-4">
    ...
    <IdentityHeader name={customer.name} company={customer.company} phone={customer.phone} email={customer.email} meta={`Customer since ${customer.createdAt} · ${customer.source}`} actions={editing ? <><Button className="gap-2 rounded-xl" onClick={save} disabled={updateMutation.isPending}><Save className="size-4" /> {updateMutation.isPending ? "Saving..." : "Save"}</Button><Button variant="ghost" className="gap-2 rounded-xl" onClick={() => { setEditing(false); setDraft(null); }} disabled={updateMutation.isPending}><X className="size-4" /> Cancel</Button></> : canEditRecord(user, customer) ? <><Button variant="outline" className="glass-soft gap-2 rounded-xl border-0" onClick={() => { setDraft(customer); setEditing(true); }}><PencilLine className="size-4" /> Edit</Button><Button className="gap-2 rounded-xl" onClick={() => navigate({ to: "/orders/new" })}><Plus className="size-4" /> Create Order</Button>{canDeleteRecord(user, customer) && <Button variant="ghost" className="gap-2 rounded-xl text-destructive" onClick={() => setConfirmDelete(true)} disabled={deleteMutation.isPending}><Trash2 className="size-4" /></Button>}</> : <Button className="gap-2 rounded-xl" onClick={() => navigate({ to: "/orders/new" })}><Plus className="size-4" /> Create Order</Button>} />
    <div className="grid gap-3 lg:grid-cols-3"><div className="glass rounded-2xl p-4 lg:col-span-2"><p className="text-sm font-semibold">Basic information</p>{editing ? <div className="mt-3 grid gap-3 sm:grid-cols-2">{([ ["name","Name"],["company","Company"],["phone","Phone"],["email","Email"],["material","Preferred material"],["source","Source"] ] as const).map(([key,label]) => <div key={key} className="space-y-1.5"><Label>{label}</Label><Input value={d[key]} onChange={(e) => upd({ [key]: e.target.value } as Partial<Customer>)} className="h-10 border-0 bg-white/70" /></div>)}<div className="space-y-1.5"><Label>Typical quantity</Label><Input type="number" min={0} value={d.quantity} onChange={(e) => upd({ quantity: Number(e.target.value) })} className="h-10 border-0 bg-white/70" /></div><div className="space-y-1.5"><Label>Units</Label><Select value={d.units} onValueChange={(v) => upd({ units: v })}><SelectTrigger className="h-10 w-full border-0 bg-white/70"><SelectValue /></SelectTrigger><SelectContent>{UNITS.map((u) => <SelectItem key={u} value={u}>{u}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1.5"><Label>Status</Label><Select value={d.status} onValueChange={(v) => upd({ status: v as Customer["status"] })}><SelectTrigger className="h-10 w-full border-0 bg-white/70"><SelectValue /></SelectTrigger><SelectContent>{STATUSES.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1.5"><Label>Assigned employee</Label><Select value={d.employeeId} onValueChange={(v) => upd({ employeeId: v })}><SelectTrigger className="h-10 w-full border-0 bg-white/70"><SelectValue /></SelectTrigger><SelectContent>{employees.map((e) => <SelectItem key={e.id} value={e.id}>{e.name}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1.5 sm:col-span-2"><Label>Address</Label><Input value={d.address} onChange={(e) => upd({ address: e.target.value })} className="h-10 border-0 bg-white/70" /></div><div className="space-y-1.5 sm:col-span-2"><Label>Feedback</Label><Textarea rows={3} value={d.feedback} onChange={(e) => upd({ feedback: e.target.value })} className="border-0 bg-white/70" /></div><div className="space-y-1.5 sm:col-span-2"><Label>Notes</Label><Textarea rows={4} value={d.notes} onChange={(e) => upd({ notes: e.target.value })} className="border-0 bg-white/70" /></div></div> : <div className="mt-3 grid gap-2 sm:grid-cols-2"><CopyField kind="phone" label="Phone" value={customer.phone} /><CopyField kind="email" label="Email" value={customer.email} /><Field label="Company" value={customer.company} /><Field label="Source" value={customer.source} /><Field label="Preferred material" value={customer.material} /><Field label="Typical quantity" value={`${customer.quantity} ${customer.units}`} /><EmployeeLink employeeId={customer.employeeId} /><Field label="Duration" value={customer.duration} /><div className="sm:col-span-2"><Field label="Address" value={customer.address} /></div></div>}</div>
    ...
    <FollowUpTimeline items={[]} target={{ id: customer.id, name: customer.name, type: "Customer" }} title="Follow-up history" />
  ...
  </div><ConfirmDialog open={confirmDelete} onOpenChange={setConfirmDelete} title={`Delete ${customer.name}?`} description="The customer cannot be deleted while orders exist." confirmLabel="Delete customer" destructive onConfirm={() => deleteMutation.mutate()} /></AppShell>;
}
