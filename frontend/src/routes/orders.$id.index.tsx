...
export const Route = createFileRoute("/orders/$id/")({
  head: () => ({
    meta: [{ title: "Order details · Saravana Traders CRM" }],
  }),
...
function OrderDetail() {
  ...
  const canEdit = canEditRecord(user, order);
  const canDelete = canDeleteRecord(user, order);
    ...
    <AppShell>
      <div className="space-y-4">
        ...
        <IdentityHeader
          ...
          phone=""
          email=""
          meta={`Order created ${order.createdAt}`}
          actions={
            canEdit ? (
              <>
                <Button
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          <div className="glass rounded-2xl p-4 lg:col-span-2">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              ...
              <Field label="Material type" value={order.materialType} />
              <Field
                label="Quantity"
                value={`${order.quantity} ${order.units}`}
              />
              <Field label="Unit price" value={inr(order.price)} />
              <Field label="Order value" value={inr(order.value)} />
              <Field label="Delivery date" value={order.deliveryDate} />
              <Field label="Created" value={order.createdAt} />
              ...
              <Field label="Assigned employee" value={order.employeeId} />
              ...
              <div className="sm:col-span-2">
                ...
                <p className="mt-1 rounded-xl bg-white/55 px-3 py-2.5 text-sm whitespace-pre-wrap">
        ...
        <ConfirmDialog
          ...
          title={`Delete ${order.invoiceNumber}?`}
