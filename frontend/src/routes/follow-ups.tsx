...
function FollowUpsPage() {
  ...
  const query = useQuery({ queryKey: ["follow-ups"], queryFn: getFollowUps, enabled: Boolean(user) });
  ...
  const rows = useMemo(() => [...(query.data ?? [])]
    .filter((followUp) => followUp.relatedType === filter)
    .sort((a, b) => {
      const statusRank = (value: string) => ({ Pending: 1, Missed: 2, Important: 3, Meeting: 4, Order: 5, Reminder: 6, Completed: 7 }[value] ?? 99);
      return statusRank(a.status) - statusRank(b.status) || `${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`);
    }), [query.data, filter]);
    ...
    <AppShell>
      <div className="space-y-4">
        <PageHeader
          ...
          actions={<Button className="gap-2 rounded-xl" onClick={() => setEditing({} as FollowUp)}><CalendarPlus className="size-4" />Add Follow-up</Button>}
        />
        ...
        <FollowUpFormDialogApi open={Boolean(editing)} onOpenChange={(open) => { if (!open) setEditing(null); }} existing={editing && editing.id ? editing : null} />
        <ConfirmDialog open={Boolean(deleteTarget)} onOpenChange={(open) => { if (!open) setDeleteTarget(null); }} title="Delete this follow-up?" description="This follow-up will be permanently removed." confirmLabel={deleteMutation.isPending ? "Deleting..." : "Delete follow-up"} destructive onConfirm={() => { if (deleteTarget) deleteMutation.mutate(deleteTarget.id); }} />
      </div>
    </AppShell>
  );
}
