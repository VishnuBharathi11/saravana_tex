...
function DashboardPage() {
  ...
  const deleteMutation = useMutation({ mutationFn: (id: string) => deleteOrder(id), onSuccess: (_data, id) => { queryClient.setQueryData<Order[]>(["orders"], (current) => current?.filter((order) => order.id !== id) ?? current); setConfirmDelete(false); setDeleteTargetId(null); toast.success("Order deleted"); void queryClient.invalidateQueries({ queryKey: ["orders"] }); void queryClient.invalidateQueries({ queryKey: ["follow-ups"] }); void queryClient.invalidateQueries({ queryKey: ["dashboard", "follow-ups"] }); void queryClient.invalidateQueries({ queryKey: ["dashboard", "summary"] }); }, onError: (e) => { setConfirmDelete(false); toast.error(e instanceof Error ? e.message : "Unable to delete order"); } });
  const summary = summaryQuery.data; const leads = leadsQuery.data ?? []; const customers = customersQuery.data ?? []; const orders = ordersQuery.data ?? []; const employees = employeesQuery.data ?? []; const followUps = followUpsQuery.data ?? []; const employeeName = (id: string) => employees.find((e) => e.id === id)?.name ?? "Unassigned"; const relatedName = (type: "Lead" | "Customer" | "Order", id: string) => type === "Lead" ? leads.find((l) => l.id === id)?.company || leads.find((l) => l.id === id)?.name || id : type === "Customer" ? customers.find((c) => c.id === id)?.company || customers.find((c) => c.id === id)?.name || id : orders.find((o) => o.id === id)?.id ?? id;
  ...
  const boardColumns: Column<Lead>[] = board === "orders" ? [{ key: "phone", header: "Order ID" }, { key: "name", header: "Customer" }, { key: "company", header: "Company" }, { key: "material", header: "Items" }, { key: "feedback", header: "Status", value: (r) => r.feedback, sortValue: (r) => ({ Pending: 1, Confirmed: 2, Cancel: 3 }[r.feedback] ?? 99), render: (r) => {
    ...
    } }, { key: "duration", header: "Delivery", render: (r) => formatDate(r.duration) }, { key: "employeeId", header: "Assigned Employee", value: (r) => employeeName(r.employeeId), render: (r) => employeeName(r.employeeId) }, { key: "createdAt", header: "Created", render: (r) => formatDateTime(r.createdAt) }, { key: "actions", header: "", render: (r) => { const order = orders.find((o) => o.id === r.id); if (!order || !canDeleteRecord(user, order)) return null; return <Button variant="ghost" size="icon" className="text-destructive" onClick={(e) => { e.stopPropagation(); setDeleteTargetId(order.id); setConfirmDelete(true); }}><Trash2 className="size-4" /></Button>; } }] : [{ key: "name", header: "Name" }, { key: "phone", header: "Contact" }, { key: "company", header: "Company" }, { key: "material", header: "Material Required" }, { key: "quantity", header: "Quantity" }, { key: "units", header: "Units" }, { key: "duration", header: "Duration" }, { key: "status", header: "Status", render: (r) => <StatusChip value={r.status} /> }, { key: "employeeId", header: "Assigned Employee", value: (r) => employeeName(r.employeeId), render: (r) => employeeName(r.employeeId) }, { key: "feedback", header: "Feedback", className: "max-w-[240px] truncate" }, { key: "followup", header: "Follow-up", render: (r) => <span className="text-xs text-muted-foreground">{(() => { const followUp = followUps.find((f) => f.relatedId === r.id); return followUp ? `${formatDate(followUp.date)} · ${formatTime(followUp.time)}` : "Not scheduled"; })()}</span> }];
