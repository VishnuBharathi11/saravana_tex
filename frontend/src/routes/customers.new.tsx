...
function NewCustomer() {
  ...
  const [d, setD] = useState<Partial<CreateCustomerInput>>({ status: "Active", quantity: 1, units: UNITS[0] });
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); if (!d.name?.trim() || !d.company?.trim() || !d.phone?.trim() || !d.email?.trim() || !d.address?.trim() || !d.material?.trim() || !d.units || !d.quantity || d.quantity <= 0 || !d.duration?.trim() || !d.source?.trim()) { toast.error("Complete all required customer fields"); return; } mutation.mutate({ name: d.name.trim(), company: d.company.trim(), phone: d.phone.trim(), email: d.email.trim(), address: d.address.trim(), material: d.material.trim(), units: d.units, quantity: d.quantity, duration: d.duration.trim(), notes: d.notes?.trim() ?? "", status: d.status, source: d.source.trim(), feedback: d.feedback?.trim() ?? "", ...(user.role === "Admin" && d.employeeId ? { employeeId: d.employeeId } : {}) }); };
