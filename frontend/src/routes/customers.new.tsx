...
export const Route = createFileRoute("/customers/new")({
  head: () => ({
    meta: [{ title: "New customer · Saravana Traders CRM" }],
  }),
...
function NewCustomer() {
  ...
  const [d, setD] = useState<Partial<CreateCustomerInput>>({
    status: "Active",
    quantity: 1,
  ...
  const mutation = useMutation({
    ...
    onError: (e) =>
      toast.error(
        e instanceof Error ? e.message : "Unable to create customer",
      ),
  });

  if (!user) return null;

  const upd = (patch: Partial<CreateCustomerInput>) =>
    setD((p) => ({ ...p, ...patch }));
    ...
    <AppShell>
      <div className="space-y-4">
        ...
        <PageHeader
          ...
          subtitle="Capture a new customer account"
        />
