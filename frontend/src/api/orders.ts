...
export interface CreateOrderInput { invoiceNumber: string; customerId: string; items: CreateOrderItemInput[]; material?: string; materialType?: string; quantity?: number; units?: string; price?: number; paymentStatus?: PaymentStatus; status?: OrderStatus; employeeId?: string; deliveryDate: string; address: string; notes?: string; }
export type UpdateOrderInput = Partial<Omit<CreateOrderInput, "items">> & { items?: CreateOrderItemInput[] };
