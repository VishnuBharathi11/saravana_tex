...
export interface Lead { id: string; name: string; company: string; phone: string; email: string; address: string; material: string; units: string; quantity: number; duration: string; notes: string; employeeId: string; status: LeadStatus; source: string; createdAt: string; feedback: string; priority: Priority; convertedCustomerId?: string; }
export interface Customer extends Omit<Lead, "status" | "priority"> { status: "Active" | "Inactive" | "Dormant" | "VIP"; totalOrders: number; totalValue: number; }
