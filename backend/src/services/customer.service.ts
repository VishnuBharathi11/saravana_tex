...
export function toCustomerResponse(row: CustomerRecord, totals: { totalOrders: number; totalValue: number }) { return { id: row.id, name: row.name, company: row.company, phone: row.phone, email: row.email, address: row.address, material: row.material, units: row.units, quantity: row.quantity, duration: row.duration, notes: row.notes, employeeId: row.employee_id, status: row.activity_status, source: row.source, createdAt: row.created_at, feedback: row.feedback, totalOrders: totals.totalOrders, totalValue: totals.totalValue }; }
