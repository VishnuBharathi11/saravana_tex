...
export async function createCustomer(db: D1Database, employee: AuthenticatedEmployee, input: CreateCustomerInput) {
  ...
  await db.prepare(`INSERT INTO customers (id, name, company, phone, email, address, material, units, quantity, duration, notes, employee_id, status, source, created_at, feedback) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, input.name, input.company, input.phone, input.email, input.address, input.material, input.units, input.quantity, input.duration, input.notes, assignedEmployeeId, input.status, input.source, createdAt, input.feedback).run();
