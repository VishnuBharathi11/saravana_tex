...
orders.get('/', async (c) => {
  ...
  const data = await Promise.all(rows.map(async (row) => { const order = await getOrderWithCustomer(db, row.id); return order!; }));
...
orders.patch('/:id', async (c) => {
  ...
  if (data.customerId !== undefined) {
    const customer = await db.prepare(`SELECT id FROM customers WHERE id = ? LIMIT 1`).bind(data.customerId).first<{ id: string }>();
    if (!customer) return c.json({ success: false, message: 'Customer not found' }, 400); customerId = customer.id;
  }
  let employeeId = existing.employee_id;
  if (employee.role === 'Admin' && data.employeeId !== undefined) {
    const assigned = await db.prepare(`SELECT id FROM employees WHERE id = ? AND status = 'Active' LIMIT 1`).bind(data.employeeId).first<{ id: string }>();
    if (!assigned) return c.json({ success: false, message: 'Assigned employee not found or inactive' }, 400); employeeId = assigned.id;
  }
  ...
  const updated = {
    ...
    material: legacy?.material ?? data.material ?? existing.material,
    materialType: legacy?.materialType ?? data.materialType ?? existing.material_type,
    quantity: legacy?.quantity ?? data.quantity ?? existing.quantity,
    units: legacy?.units ?? data.units ?? existing.units,
    price: legacy?.price ?? data.price ?? existing.price,
  ...
  try {
    await db.prepare(`UPDATE orders SET invoice_number = ?, customer_id = ?, material = ?, material_type = ?, quantity = ?, units = ?, price = ?, payment_status = ?, status = ?, employee_id = ?, delivery_date = ?, address = ?, notes = ? WHERE id = ?`).bind(updated.invoiceNumber, customerId, updated.material, updated.materialType, updated.quantity, updated.units, updated.quantity, updated.price, updated.paymentStatus, updated.status, employeeId, updated.deliveryDate, updated.address, updated.notes, id).run();
